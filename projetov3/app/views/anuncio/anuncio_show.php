...
$jaCanditatou = $jaCanditatou ?? false;
...
$descricao = htmlspecialchars($anuncio->getDescricao(), ENT_QUOTES, 'UTF-8');
...
$duracao = htmlspecialchars($anuncio->getDuracao() ?? '', ENT_QUOTES, 'UTF-8');
$observacoes = htmlspecialchars($anuncio->getObservacoes() ?? '', ENT_QUOTES, 'UTF-8');

$statusBadge = $status === 'ABERTO' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800';
$isTrabalhadore = $usuarioLogado && $usuarioLogado->isTrabalhador();
$isContratante = $usuarioLogado && $usuarioLogado->isContratante();
$isProprietario = $isContratante && $usuarioLogado->getIdUsuario() === $anuncio->getIdContratante();
?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="flex mb-8" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                <li><a href="/anuncios" class="text-blue-600 hover:text-blue-700 font-medium">Vagas</a></li>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-gray-900 font-medium"><?= $duracao ?: 'A definir' ?></p>
                        </div>
                    </div>
                </div>
                ...
                <?php if ($observacoes): ?>
                    <div class="bg-blue-50 rounded-xl border border-blue-200 p-6">
                        <h3 class="text-sm font-bold text-blue-900 mb-2">📌 Observações importantes</h3>
                        <p class="text-blue-800 text-sm"><?= $observacoes ?></p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6 sticky top-24">
                    ...
                    <!-- Ação Principal -->
                    <?php if ($isTrabalhadore && $status === 'ABERTO' && !$jaCanditatou): ?>
                        <form method="POST" action="/candidatura/criar" class="mb-4">
                    ...
                    <?php if ($isProprietario): ?>
                        <div class="space-y-2">
                            <a href="/anuncios/editar?id=<?= $anuncio->getId() ?>" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg text-center transition-colors duration-200 block">
                                Editar
                            </a>
                            <form method="POST" action="/anuncios/excluir" class="block">
                                <input type="hidden" name="id_anuncio" value="<?= $anuncio->getId() ?>">
                    ...
                    <?php if ($usuarioLogado && !$isProprietario): ?>
                        <div class="pt-4 border-t border-gray-200 mt-4">
                            <a href="/denuncia/criar?id=<?= $anuncio->getIdContratante() ?>" class="w-full bg-white border border-red-200 hover:bg-red-50 text-red-600 font-semibold py-2 px-4 rounded-lg text-center transition-colors duration-200 block">
                    ...
                    <?php if ($isProprietario): ?>
                        <div class="pt-4 border-t border-gray-200 mt-4">
                            <a href="/candidatura/candidatos?id=<?= $anuncio->getId() ?>" class="text-blue-600 hover:text-blue-700 font-semibold text-sm">
                                Ver candidatos →
                            </a>
                        </div>
                    <?php endif; ?>
