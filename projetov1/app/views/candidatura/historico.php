    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-wrap gap-2 mb-8 border-b border-gray-200">
            ...
            <?php if ($isContratante): ?>
                <button onclick="mostrarAba('meus-anuncios')" class="tab-btn px-4 py-2 font-semibold text-gray-600 border-b-2 border-transparent hover:text-blue-600">
                    Meus Anúncios
                </button>
            <?php endif; ?>
        </div>
        ...
        <div id="minhas-candidaturas" class="tab-content">
            ...
            <?php if (empty($candidaturas)): ?>
                <div class="text-center py-16 bg-white rounded-xl border border-gray-100">
                    ...
                    <a href="/anuncios" class="text-blue-600 hover:text-blue-700 font-semibold mt-2 inline-block">Explorar vagas →</a>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($candidaturas as $item):
                        ...
                        $tituloAnuncio = $anuncio ? $anuncio->getTitulo() : 'Anúncio indisponível';
                    ...
                    ?>
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 candidatura-card status-<?= strtolower($status) ?>">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    <h3 class="font-bold text-gray-900"><?= htmlspecialchars($tituloAnuncio, ENT_QUOTES, 'UTF-8') ?></h3>
                            ...
                            <div class="flex items-center justify-between mb-4 py-4 border-y border-gray-200">
                                <div>
                                    ...
                                    <p class="text-lg font-bold text-green-600"><?= $anuncio ? 'R$ ' . number_format($anuncio->getRemuneracao(), 2, ',', '.') : 'N/D' ?></p>
                                </div>
                                <div class="text-right">
                                    <p class="text-xs text-gray-600">Tipo</p>
                                    <p class="font-semibold text-gray-900"><?= $anuncio ? ($anuncio->getTipoServico() === 'TEMPORARIO' ? 'Temporário' : 'Fixo') : 'N/D' ?></p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2 text-sm mb-4 text-gray-600">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span><?= htmlspecialchars($anuncio?->getLocalizacao() ?? 'Não informado', ENT_QUOTES, 'UTF-8') ?></span>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                <?php if ($anuncio): ?>
                                    <a href="/anuncios/visualizar?id=<?= $anuncio->getId() ?>" class="block text-center bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium py-2 rounded-lg text-sm transition-colors">
                                        Ver vaga
                                    </a>
                                <?php else: ?>
        ...
        <?php if ($isContratante): ?>
            <div id="meus-anuncios" class="tab-content hidden">
                <!-- Conteúdo específico para anúncios do contratante -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 text-center">
                    <p class="text-gray-600">Gerencie seus anúncios em <a href="/anuncios" class="text-blue-600 hover:text-blue-700 font-semibold">Minhas Vagas</a></p>
                </div>
            </div>
        <?php endif; ?>
    </div>
